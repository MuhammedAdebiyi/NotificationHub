import type { ReactNode } from 'react'
import { NavLink, useNavigate } from 'react-router-dom'
import { authService } from '@/shared/services/auth.service'

const navItems = [
  { to: '/dashboard', label: 'Dashboard' },
  { to: '/notifications', label: 'Notifications' },
  { to: '/templates', label: 'Templates' },
  { to: '/campaigns', label: 'Campaigns' },
  { to: '/analytics', label: 'Analytics' },
  { to: '/logs', label: 'Logs' },
  { to: '/webhooks', label: 'Webhooks' },
  { to: '/developer-analytics', label: 'Developer Analytics' },
  { to: '/users', label: 'Team' },
  { to: '/settings', label: 'Settings' },
]

export default function AppLayout({ children }: { children: ReactNode }) {
  const navigate = useNavigate()

  function handleLogout() {
    authService.clearToken()
    navigate('/login')
  }

  return (
    <div className="min-h-screen bg-paper text-ink flex">
      <aside className="w-64 shrink-0 border-r border-ink/10 bg-fog/40 flex flex-col">
        <div className="px-6 py-6">
          <a href="/dashboard" className="font-display font-extrabold text-lg tracking-tight">
            NotificationHub<span className="text-violet">.</span>
          </a>
        </div>

        <nav className="flex-1 px-3 space-y-1">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) =>
                `block px-4 py-2.5 rounded-lg text-sm font-medium transition ${
                  isActive
                    ? 'bg-ink text-white'
                    : 'text-ink/70 hover:bg-fog hover:text-ink'
                }`
              }
            >
              {item.label}
            </NavLink>
          ))}
        </nav>

        <div className="px-6 py-5 border-t border-ink/10 space-y-3">
          <span className="text-xs font-medium px-3 py-1.5 rounded-full bg-teal/10 text-teal">● Live</span>
          <button
            onClick={handleLogout}
            className="w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium text-ink/70 hover:bg-red-50 hover:text-red-600 transition"
          >
            Sign out
          </button>
        </div>
      </aside>

      <main className="flex-1 px-6 sm:px-10 py-8 overflow-y-auto">
        {children}
      </main>
    </div>
  )
}