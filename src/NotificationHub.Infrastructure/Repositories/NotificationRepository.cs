...
public class NotificationRepository : INotificationRepository
{
    ...
    public async Task<Notification?> GetByPublicIdAsync(
        Guid organizationId, Guid publicId,
        CancellationToken cancellationToken = default)
        => await _context.Notifications
            .FirstOrDefaultAsync(
                n => n.OrganizationId == organizationId && n.PublicId == publicId,
                cancellationToken);

    public async Task<bool> IdempotencyKeyExistsAsync(
        Guid organizationId, string key,
        CancellationToken cancellationToken = default)
        => await _context.IdempotencyKeys
            .AnyAsync(k => k.OrganizationId == organizationId && k.Key == key, cancellationToken);
