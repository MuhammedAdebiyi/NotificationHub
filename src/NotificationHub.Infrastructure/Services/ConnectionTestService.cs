        ...
        switch (protocol)
        {
            case SqlProtocol.Postgres:
                await using (var conn = new NpgsqlConnection(connectionString))
                {
                    ...
                    await using var cmd = new NpgsqlCommand(ProbeQuery, conn) { CommandTimeout = CommandTimeoutSeconds };
            ...
            case SqlProtocol.MySql:
                ...
                {
                    await conn.OpenAsync(cancellationToken);
                    await using var cmd = new MySqlCommand(ProbeQuery, conn) { CommandTimeout = CommandTimeoutSeconds };
                    await cmd.ExecuteScalarAsync(cancellationToken);
                }
                break;
            ...
            case SqlProtocol.SqlServer:
                ...
                {
                    await conn.OpenAsync(cancellationToken);
                    await using var cmd = new SqlCommand(ProbeQuery, conn) { CommandTimeout = CommandTimeoutSeconds };
                    await cmd.ExecuteScalarAsync(cancellationToken);
                }
                break;
        }
